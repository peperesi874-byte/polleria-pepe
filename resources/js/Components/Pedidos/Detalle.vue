...
const props = defineProps({
  base: { type: String, required: true },           // 'admin' | 'vendedor'
...
function setEstado(estado){
  if(bloqueado.value || estado===estadoActual.value) return
  processing.value = true
  router.put(route(`${props.base}.pedidos.estado`, props.pedido.id),
    {estado},
...
function doCancelar(){
  ...
  processing.value = true
  router.put(route(`${props.base}.pedidos.cancelar`, props.pedido.id),
    {motivo},
    {preserveScroll:true,preserveState:true,onFinish:()=>processing.value=false})
}
function asignar(val){
  ...
  processing.value = true
  router.put(route(`${props.base}.pedidos.asignar`, props.pedido.id),
    {repartidor_id: val?Number(val):null},
    {preserveScroll:true,preserveState:true,onFinish:()=>processing.value=false})
}
</script>
...
<template>
  <div class="max-w-5xl mx-auto px-6 py-8">
    <div class="mb-6 flex items-center justify-between gap-3">
      ...
      <Link :href="route(`${base}.pedidos.index`)" class="text-indigo-600 hover:text-indigo-700 font-medium">
        ← Volver a la lista
      </Link>
    </div>
    ...
    <div class="mb-6 grid gap-4 md:grid-cols-2">
      ...
      <div class="rounded-xl border bg-white p-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <div class="relative">
            <select class="w-56 rounded-lg border border-gray-300 bg-white px-3 py-2 pr-9 text-sm
    ...
    <div class="grid md:grid-cols-4 gap-4 mb-6">
      <div class="rounded-xl border bg-white p-4"><p class="text-sm text-gray-500">Estado</p>
        <p class="text-lg font-semibold capitalize">{{ pedido.estado.replace('_',' ') }}</p></div>
      <div class="rounded-xl border bg-white p-4"><p class="text-sm text-gray-500">Tipo de entrega</p>
        <p class="text-lg font-semibold capitalize">{{ pedido.tipo.replace('_',' ') }}</p></div>
      <div class="rounded-xl border bg-white p-4"><p class="text-sm text-gray-500">Total</p>
        <p class="text-lg font-semibold">{{ money(pedido.total) }}</p></div>
      <div class="rounded-xl border bg-white p-4"><p class="text-sm text-gray-500">Asignado a</p>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      <table class="w-full text-sm">
        <thead class="bg-indigo-50 text-indigo-800 text-xs uppercase">
          <tr><th class="px-4 py-3 text-left">Producto</th>
              <th class="px-4 py-3 text-left">Cantidad</th>
              <th class="px-4 py-3 text-left">Precio</th>
              <th class="px-4 py-3 text-left">Subtotal</th></tr>
        </thead>
        <tbody>
