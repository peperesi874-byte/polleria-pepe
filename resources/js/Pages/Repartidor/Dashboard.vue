...
const props = defineProps({
  stats: { type: Object, default: () => ({ asignados:0, pendientes:0, en_camino:0, entregados_hoy:0 }) },
  proximos: { type: Array, default: () => [] },
  enRuta: { type: Array, default: () => [] },
...
const fmtDateTime = v => (v ? new Date(v).toLocaleString('es-MX') : '')
...
<template>
  <AuthenticatedLayout>
    <div class="max-w-6xl mx-auto p-6 space-y-8">
      <!-- HEADER (igual estilo que vendedor) -->
        ...
        <div class="flex flex-col items-end gap-1 text-right">
          ...
          <div class="mt-2 flex gap-2">
            <Link
              ...
              class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-sky-50 text-sky-700 border border-sky-100 hover:bg-sky-100 transition"
            >
      ...
      <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-3">
          ...
          >
            No tienes pedidos pendientes ni listos para reparto.
          </div>
            ...
            <table class="min-w-full text-sm">
              <thead class="bg-neutral-50 text-neutral-500 text-xs uppercase tracking-wide">
                <tr>
                  ...
                  <th class="px-4 py-2 text-left hidden md:table-cell">Colonia</th>
              ...
              <tbody>
                <tr
                  ...
                  :key="p.id"
                  class="border-t hover:bg-neutral-50/70 transition-colors"
                >
                  <td class="px-4 py-2 font-medium text-neutral-900">
                    {{ p.folio ?? ('PED-' + p.id) }}
                  </td>
                  <td class="px-4 py-2 text-xs text-neutral-500 hidden md:table-cell">
        ...
        <div class="space-y-6">
          ...
          <div class="rounded-2xl border bg-white shadow-sm p-4 space-y-3">
            ...
            <div v-if="!props.enRuta.length" class="text-xs text-neutral-500">
            ...
            <ul v-else class="space-y-2 text-sm">
              <li
                ...
                :key="p.id"
                class="flex items-center justify-between gap-2 border-b last:border-b-0 pb-2 last:pb-0"
              >
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-neutral-500 truncate">
                    {{ [p.entrega_colonia, p.entrega_municipio].filter(Boolean).join(', ') || 'Sin datos de zona' }}
                  </p>
                </div>
                <div class="text-right">
                  <p class="text-xs text-neutral-400">{{ fmtMoney(p.total) }}</p>
                  <span
          ...
          <div class="rounded-2xl border bg-white shadow-sm p-4 space-y-3">
            ...
            <div v-if="!props.historial.length" class="text-xs text-neutral-500">
              Aún no hay entregas o cancelaciones recientes.
            </div>

            <ul v-else class="space-y-2 text-sm max-h-64 overflow-auto pr-1">
              <li
                v-for="p in props.historial"
                :key="p.id"
                class="flex items-center justify-between gap-3 border-b last:border-b-0 pb-2 last:pb-0"
              >
                <div class="flex-1 min-w-0">
                  <p class="font-medium text-neutral-900 truncate">
                    Pedido #{{ p.folio ?? p.id }}
