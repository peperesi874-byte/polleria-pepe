...
const props = defineProps({
  role: { type: String, default: 'admin' },   // ⬅️ El controlador manda 'role'
...
function setEstado(estado) {
  ...
  router.put(
    route(base.value + '.pedidos.estado', props.pedido.id),
    { estado },
...
function doCancelar() {
  ...
  processing.value = true
  router.put(
    route(base.value + '.pedidos.cancelar', props.pedido.id),
    { motivo },
    { preserveScroll: true, preserveState: true, onFinish: () => (processing.value = false) }
  )
}
...
function asignar(val) {
  ...
  processing.value = true
  router.put(
    route(base.value + '.pedidos.asignar', props.pedido.id),
